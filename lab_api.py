...
_user_decks_dir = _Path(__file__).parent / "user-decks"
...
# Mount user-decks/ so exported .dck files are fetchable at /user-decks/{file}.dck
if _user_decks_dir.exists():
    app.mount("/user-decks", StaticFiles(directory=str(_user_decks_dir)), name="user-decks")
...
def _resolve_forge_dir() -> str:
    ...
    for candidate in [
        ...
        project_root.parent / "forge-repo",           # legacy fallback
        project_root.parent / "forge-repo" / "forge-gui",
...
def _resolve_forge_jar() -> str:
    ...
    candidates = [
        ...
        project_root.parent / "forge-repo" / "forge-gui-desktop" / "target",  # legacy fallback
